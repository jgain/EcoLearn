...
def create_specratios(basedir):
    ...
    tabmodels =  {0:[(0,0,0.000,10.0,1.000,0.210,'Oxalis_oregano_A001','', 6.0)],\
    ...
    modmatrix = [moddetails for modid, moddetails in all_models.items()]
    modmatrix.sort(key=lambda x: x[0])
    ...
    with open(os.path.join(basedir, "modelDetails.csv"), "w+") as outfile:
        outfile.write(csvstr)

    with open(os.path.join(basedir, "modelMapping.csv"), "w+") as outfile:
