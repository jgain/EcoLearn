...
const bool SYMBOLIC_RENDERER = true;
...
class TRenderer
{
 ...
 private:
    ...
    std::vector<GLuint> miscTextures;	// added by Kp Kapp: miscellaneous textures
    GLuint miscTexturesStartingLocation;
   ...
   GLfloat screenQuad[16] = {-1.0f, -1.0f,   0.0f, 0.0f,
    ...
    void drawManipulatorsInstanced(GLuint program, bool drawTO_FB=false);
...
public:
    ...
    void setMiscTextures(const std::vector<GLuint> &texture_ids)
    {
        miscTextures = texture_ids;
    }

    void setMiscTexturesStartingLocation(GLuint loc)
    {
        miscTexturesStartingLocation = loc;
    }
   ...
   // set directional light N {0,1}(radiance scaling)
    ...
    void drawManipulators(GLuint programID, bool drawToFB, bool brush);
};
