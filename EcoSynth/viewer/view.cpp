...
void View::startPan(float u, float v)
{
    bp_u = u;
    bp_v = v;
}

void View::pan(float u, float v)
{
    prevfocus = currfocus;
    ...
    cop.x += bp_u - u, cop.z += bp_v - v;
    currfocus.x += bp_u - u, currfocus.z += bp_v - v;

    bp_u = u, bp_v = v;
}

void View::setForcedLocation(float x, float y, float z)
{
    cop.x = x, cop.y = y, cop.z = z;
    currfocus.x = x, currfocus.y = y, currfocus.z = z + 1.0f;
}

void View::modLocation(float x, float y, float z)
{
    currfocus.x += x, currfocus.y += y, currfocus.z += z;
}
...
void View::sundir(Vector sunvec)
{
    ...
    if(!(initvec == nsunvec)) // do nothing when sun oriented with initial
    {
        ...
        // cerr << "bivec = " << bivec.i << ", " << bivec.j << ", " << bivec.k << endl;
        // cerr << "biangle = " << biangle << endl;
...
glm::mat4x4 View::getProjMtx()
{
    ...
    if(viewtype == ViewState::PERSPECTIVE)
    {
        ...
        maxx = mult * ACTUAL_ASPECT;
        //projMx = glm::frustum(minx, maxx, -8.0f, 8.0f, 50.0f, 100000.0f);
        projMx = glm::frustum(minx, maxx, -mult, mult, mindist, maxdist);
    }
