...
private:
     ...
     */
    bool close(int k, std::vector<std::array<float,ndim>> &clusters, float tol);
     ...
     */
    float kdist(std::array<float,ndim> f1, std::array<float,ndim> f2);

public:
    
    kMeans(int kmaxiter) : kmaxiter(kmaxiter)
    {}
     ...
     */
    float cluster(std::vector<std::array<float,ndim>> &features, int k, std::vector<int> &assigns, std::vector<std::array<float,ndim>> &clusters);
     ...
     * @param clusters  the mean values for each cluster
     */
    void search(std::vector<std::array<float,ndim>> &features, int &k, std::vector<int> &assigns, std::vector<std::array<float,ndim>> &clusters);
