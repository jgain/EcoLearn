...
WaterFill::WaterFill(const ValueMap<float> &tgrid, int twidth, int theight)
    : tgrid(tgrid), twidth(twidth), theight(theight)
{
...
{
    return flow[x+y*gwidth] >absorbsion;
}

float WaterFill::riverMoisture(uint x, uint y)
{
    return float(river_side[x + y*gwidth]) / float(absorbsion);
}
...
void WaterFill::compute(uint step)
{
    ...
    wf_point shift [4];
    shift[0] = wf_point(-1, 0);
    shift[1] = wf_point(1, 0);
    shift[2] = wf_point(0, -1);
    shift[3] = wf_point(0, 1);
    ...
    for(uint x = 0; x< dx; ++x)
        for(uint y = 0; y< dy; ++y)
        {
            ...
            wf_point p(x, y);
            wf_point nb = p;
            double this_h = (double) getHeight((int) x, (int) y);
            double h = this_h;
            shuffle<wf_point, 4>(shift);
            for(int j =0; j<4; ++j)
            {
                wf_point cur = p + shift[j];
                if(cur.x()<0.0 || cur.x() >= (int) dx || cur.y()<0.0 || cur.y() >= (int) dy)
                    continue;

                double ch = (double) getHeight((int) cur.x(), (int) cur.y());
                if(h > ch)
                {
                    nb = cur;
                    h = ch;
    ...
    while(!parse_stack.empty())
    {
        ...
        //std::cout << "in parse stack" << std::endl;
    ...
    {
        double ch = (double) getFlatHeight((int) id);
    ...
    while(true)
    {
        ...
        for(uint i = 0; i<to_resolve.size(); ++i)
        {
            ...
            while(!parse_stack.empty())
            {
                ...
                wf_point p(cur%dx, cur/dx);
                double ch = getHeight((int) p.x(), (int) p.y());

                shuffle<wf_point, 4>(shift);
                for(int j =0; j<4; ++j)
                {
                    wf_point nb = p + shift[j];
...
float WaterFill::getHeight(int x, int y)
{
    //return tgrid[y * gwidth + x];
    return tgrid.get(x, y);
}

float WaterFill::getFlatHeight(int idx)
{
    //int x = idx % gwidth;
    //int y = idx / gwidth;
    return tgrid.get(idx);
}

float WaterFill::getCellExtent()
{
	return twidth / (float)gwidth;
}

float WaterFill::getCellArea()
{
	return getCellExtent() * getCellExtent();
}

std::vector<uint16_t> WaterFill::get_16bit_img_greyscale_data()
{
	std::vector<uint16_t> img_data(gwidth * gheight);
	for (int y = 0; y < gheight; y++)
	{
		for (int x = 0; x < gwidth; x++)
		{
			int idx = y * gwidth + x;
			uint32_t val = flow[idx];
...
#ifdef IMG_WRITE
void WaterFill::write_to_greyscale_png(std::string filename)
{
	...
	auto img_data = get_16bit_img_greyscale_data();

    write_png(filename, img_data, gwidth, gheight);
}
#endif	// IMG_WRITE
...
void WaterFill::expandRivers(float max_moisture_factor, float slope_effect)
{
    ...
    float cell_width = std::sqrt(getCellArea());
    ...
    for(uint i =0; i<flow.size(); ++i)
    {
        ...
        float drainage = float(f) * precipitation * getCellArea();
    ...
    for(uint i =0; i<newFlow.size(); ++i)
    {
        ...
        for(int x = std::max(0, p_x - i_width); x <= std::min(dx-1, p_x+i_width); ++x)
            for(int y = std::max(0, p_y - i_width); y <= std::min(dy-1, p_y+i_width); ++y)
            {
                ...
                float sdx = tx*getCellExtent();
                float sdy = ty*getCellExtent();
