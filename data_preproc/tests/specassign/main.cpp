...
int main(int argc, char * argv [])
{
    ...
    */
    ...
    std::vector<float> maxheights(100.0f, 3);
    ...
    for (int i = 0; i < ntexs; i++)
    {
        ...
        if (i == 0)
        {
            optim_ptr = species_set::create_optim(&chmdata, maps_ptrs, species_perc, true, maxheights);
        }
        else
        {
            optim_ptr = species_set::create_optim(&chmdata, maps_ptrs, species_perc, solutions, true, maxheights);
            ValueMap<int> prev_w = optim_ptr->get_prev_winmap();
            int w, h;
            prev_w.getDim(w, h);
            int valid_count = 0;
            for (int y = 0; y < h; y++)
            {
                for (int x = 0; x < w; x++)
                {
                    if (prev_w.get(x, y) >= 0 && prev_w.get(x, y) < species_perc.size())
                    {
                        valid_count++;
                    }
                }
            }
            std::cout << "Num occupied by valid species: " << valid_count << std::endl;
        }
